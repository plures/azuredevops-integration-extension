...
describe('AzureDevOpsIntClient repositories & PRs', function () {
  ...
  it('getRepositories returns list', async function () {
    ...
    nock('https://dev.azure.com').get('/org/proj/_apis/git/repositories').reply(200, { value: [{ id: 'r1', name: 'repo1' }] });
  ...
  it('getPullRequests returns mapped PRs', async function () {
    const client = new AzureDevOpsIntClient('org', 'proj', 'pat');
    nock('https://dev.azure.com').get('/org/proj/_apis/git/pullrequests').query(true).reply(200, { value: [{ pullRequestId: 7, title: 'Fix' , repository: { name: 'repo1' } }] });
  ...
  it('createPullRequest returns server response', async function () {
    ...
    nock('https://dev.azure.com').post(`/org/proj/_apis/git/repositories/${repositoryId}/pullrequests`).reply(200, { pullRequestId: 99, title: 'New PR' });
    const pr = await client.createPullRequest(repositoryId, 'refs/heads/feature/x', 'refs/heads/main', 'New PR');
