...
import { __setTestContext, handleMessage } from '../src/activation';
...
describe('activation message handling', () => {
  it('handles getWorkItems by posting workItemsLoaded', async () => {
    ...
    const provider = { getWorkItems: () => [{ id: 1, fields: { 'System.Title': 'T' } }], refresh: () => {} } as any;
  ...
  it('startTimer triggers timer.start with id', async () => {
    const calls: any[] = [];
    const provider = { getWorkItems: () => [{ id: 123, fields: { 'System.Title': 'X' } }], refresh: () => {} } as any;
  ...
  it('refresh message calls provider.refresh', async () => {
    let refreshed = false;
    const provider = { refresh: (q?: any) => { refreshed = true; } } as any;
