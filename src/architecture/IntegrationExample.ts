...
export function createContextStores(webviewIntegration: WebviewContextIntegration) {
  ...
  const connections = derived(contextStore, ($context: any) => 
    $context?.connections || []
  );
  
  const activeConnection = derived(contextStore, ($context: any) => 
    $context?.connections.find((c: any) => c.id === $context.activeConnectionId) || null
  );
  
  const activeWorkItems = derived(contextStore, ($context: any) => 
    $context?.workItemsByConnection.get($context.activeConnectionId) || []
  );
  
  const timerState = derived(contextStore, ($context: any) => 
    $context?.timer || { isActive: false, isRunning: false, elapsed: 0 }
  );
  
  const isLoading = derived(contextStore, ($context: any) => 
