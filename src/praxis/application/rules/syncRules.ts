...
function updateScalarProperties(state: any, payload: any) {
  ...
  if (payload.activeConnectionId !== undefined) state.context.activeConnectionId = payload.activeConnectionId;
  ...
  if (payload.pendingWorkItems !== undefined) state.context.pendingWorkItems = payload.pendingWorkItems;
  if (payload.deviceCodeSession !== undefined) state.context.deviceCodeSession = payload.deviceCodeSession;
  if (payload.authCodeFlowSession !== undefined) state.context.authCodeFlowSession = payload.authCodeFlowSession;
  if (payload.lastError !== undefined) state.context.lastError = payload.lastError;
  if (payload.errorRecoveryAttempts !== undefined) state.context.errorRecoveryAttempts = payload.errorRecoveryAttempts;
  if (payload.workItemsError !== undefined) state.context.workItemsError = payload.workItemsError;
  if (payload.workItemsErrorConnectionId !== undefined) state.context.workItemsErrorConnectionId = payload.workItemsErrorConnectionId;
  if (payload.debugLoggingEnabled !== undefined) state.context.debugLoggingEnabled = payload.debugLoggingEnabled;
  if (payload.debugViewVisible !== undefined) state.context.debugViewVisible = payload.debugViewVisible;
}
