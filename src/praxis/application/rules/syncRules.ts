...
/**
 * Update scalar context properties from payload
 */
function updateScalarProperties(state: any, payload: any): void {
  ...
  if (payload.workItemsErrorConnectionId !== undefined) {
    state.context.workItemsErrorConnectionId =
      payload.workItemsErrorConnectionId;
  }
...
/**
 * Update Map-based context properties from payload
 */
function updateMapProperties(state: any, payload: any): void {
  if (payload.connectionStates !== undefined) {
    if (payload.connectionStates instanceof Map) {
      ...
      state.context.connectionStates = new Map(
        Object.entries(payload.connectionStates),
      );
    }
  }
  if (payload.connectionWorkItems !== undefined) {
    if (payload.connectionWorkItems instanceof Map) {
      ...
      state.context.connectionWorkItems = new Map(
        Object.entries(payload.connectionWorkItems),
      );
    }
  }
  if (payload.connectionQueries !== undefined) {
    if (payload.connectionQueries instanceof Map) {
      ...
      state.context.connectionQueries = new Map(
        Object.entries(payload.connectionQueries),
      );
    }
  }
  if (payload.connectionFilters !== undefined) {
    if (payload.connectionFilters instanceof Map) {
      ...
      state.context.connectionFilters = new Map(
        Object.entries(payload.connectionFilters),
      );
    }
  }
  if (payload.connectionViewModes !== undefined) {
    if (payload.connectionViewModes instanceof Map) {
      ...
      state.context.connectionViewModes = new Map(
        Object.entries(payload.connectionViewModes),
      );
    }
  }
  if (payload.pendingAuthReminders !== undefined) {
    if (payload.pendingAuthReminders instanceof Map) {
      state.context.pendingAuthReminders = new Map(
        payload.pendingAuthReminders,
      );
    } else if (
      ...
      state.context.pendingAuthReminders = new Map(
        Object.entries(payload.pendingAuthReminders),
      );
    }
  }
}
...
export const syncStateRule = defineRule<ApplicationEngineContext>({
  ...
  impl: (state, events) => {
    ...
    // Update Map properties with immutability
