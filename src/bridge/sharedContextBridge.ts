...
interface ApplicationContext {
  ...
  pendingAuthReminders?: Map<string, { connectionId: string; reason: string; status: string }> | Array<{ connectionId: string; reason?: string; status?: string; detail?: string; message?: string; label?: string; authMethod?: 'pat' | 'entra' }>;
...
export function createSharedContextBridge({
  ...
  function maybePostContext(reason: string) {
    ...
    try {
      ...
      const state = applicationManager && typeof applicationManager.getApplicationState === 'function'
