...
import {
  ...
  createClient,
  createProvider,
} from './functions/connection/connectionManagerHelpers.js';
import { clearEntraIdToken } from './functions/auth/authentication.js';
import { ProjectConnection } from '../praxis/connection/types.js';
...
export class ConnectionFSMManager {
  ...
  async connectToConnection(
    ...
    return this.waitForConnection(manager, config, timeoutMs, forceInteractive);
  }
      ...
      const pollInterval = setInterval(() => {
        ...
        if (state === 'authenticating' && !authStarted) {
          ...
          if (this.context) {
            ...
            this.logger.error('Extension context not set, cannot authenticate');
          ...
          createClient(manager, config, data.credential, () => {
            if (this.context && config.authMethod === 'entra') {
              clearEntraIdToken(this.context, config.tenantId, config.clientId)
                .catch((err) => {
                  this.logger.warn(`Failed to clear Entra ID token: ${err}`);
                })
                .finally(() => {
            ...
            this.logger.error(`Client creation error: ${err}`);
          });
          ...
          createProvider(manager, config, data.client as AzureDevOpsIntClient).catch((err) => {
            this.logger.error(`Provider creation error: ${err}`);
          });
        }
