/**
 * Module: src/fsm/functions/azureClientFunctions.ts
 * Owner: application
 * Reads: (document)
 * Writes: (document)
 * Receives: (document)
...
type ConnectionLogger = {
  FSMComponent: LoggerModule['FSMComponent'];
  fsmLogger: LoggerModule['fsmLogger'];
  fsmContext: FSMContext;
};

let loggerModulePromise: Promise<LoggerModule> | undefined;

async function getLoggerModule(): Promise<LoggerModule> {
...
async function getConnectionLogger(connectionId: string): Promise<ConnectionLogger> {
...
/**
 * Pure function to validate Azure client configuration
 * @param context FSM context containing connection configuration
 * @returns Validation result with normalized config
 */
export async function validateClientConfig(context: ConnectionContext): Promise<{
  isValid: boolean;
  config?: AzureClientConfig;
  errors?: string[];
  context: ConnectionContext;
  ...
  const config: AzureClientConfig = {
    organization: safeOrganization,
    project: safeProject,
    credential: context.credential!,
    options: {
      ratePerSecond: 10, // Default rate limiting
      burst: 20,
...
/**
 * Pure function to create Azure DevOps client
 * @param context FSM context with validated configuration
 * @param config Validated client configuration
...
/**
 * Pure function to test client connectivity
 * @param context FSM context
 * @param client Azure DevOps client to test
...
/**
 * Pure function to normalize connection configuration
 * @param rawConfig Raw connection configuration
 * @returns Normalized and validated configuration
 */
export function normalizeConnectionConfig(rawConfig: any): {
  isValid: boolean;
  config?: any;
  errors?: string[];
} {
...
function extractOrganizationFromUrl(raw?: string): string | undefined {
  if (!raw) {
    return undefined;
  }
