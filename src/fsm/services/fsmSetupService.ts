...
export class FSMSetupService {
  ...
  async startSetup(options?: { skipInitialChoice?: boolean }): Promise<FSMSetupResult> {
    ...
    try {
      ...
      if (!connectionInfo) {
        await vscode.window.showErrorMessage('Invalid work item URL. Please provide a valid Azure DevOps work item URL.');
        return { status: 'cancelled' };
      }
      ...
      await vscode.window.showErrorMessage(`Setup failed: ${error instanceof Error ? error.message : String(error)}`);
      return { status: 'cancelled' };
    }
  }
  ...
  private parseWorkItemUrl(url: string): { organization: string; project: string; baseUrl: string } | null {
    try {
  ...
  private async manageExistingConnections(connections: ProjectConnection[]): Promise<FSMSetupResult> {
