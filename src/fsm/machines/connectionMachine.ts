 ...
 */
async function tryRestApiTenantDiscovery(organizationUrl: string, orgName: string, context: any): Promise<string | null> {
  try {
...
export const connectionMachine = createMachine({
            ...
            interactive_auth: {
              invoke: {
                ...
                onError: {
                  target: '#connection.auth_failed',
                  actions: assign({
                    lastError: ({ event }) => (event.error as Error)?.message || 'Entra authentication failed',
                    retryCount: ({ context }) => context.retryCount + 1,
                    reauthInProgress: false,
                    forceInteractive: () => false,
                  }),
  ...
  const clientId = AZURE_DEVOPS_PUBLIC_CLIENT_ID;
        ...
        logger.debug('Final tenant selection for device code authentication', authContext, {
          ...
          discoveredTenant,
          finalTenantId,
          selectionReason: discoveredTenant ? 'discovery' : 
                           (!input.config?.tenantId || tenantId === 'organizations') ? 'organizations-fallback' : 'user-config',
        ...
        const authProvider = new EntraAuthProvider({
          ...
          deviceCodeCallback: async (deviceCode: string, userCode: string, verificationUri: string, expiresIn: number) => {
            const authContext = { component: FSMComponent.AUTH, connectionId: input.connectionId };
