...
export type ApplicationContext = {
  ...
  // UI components
  webviewPanel?: any;
  pendingWorkItems?: { workItems: any[]; connectionId?: string; query?: string; kanbanView?: boolean; types?: string[] };
...
export type ApplicationEvent =
  ...
  | { type: 'REFRESH_DATA' }
  | { type: 'WORK_ITEMS_LOADED'; workItems: any[]; connectionId?: string; query?: string; kanbanView?: boolean; types?: string[] }
...
function isSetupUICompletionEvent(event: unknown): event is { type: 'done.invoke.setupUI'; output: SetupUIResult } {
...
function deriveLegacyFallback(
  ...
  const organization = readConfigString(config, 'organization') ?? readConfigString(legacyConfig, 'organization');
    ...
    creating_provider: {
      invoke: {
        ...
        onError: {
          target: 'connection_failed',
          actions: assign({
            lastError: ({ event }) => (event.error as Error)?.message || 'Provider creation failed',
          }),
        },
      },
    },
...
export const authMachine = createMachine({
    ...
    pat_auth: {
      invoke: {
        ...
        onError: {
          target: 'auth_failed',
          actions: assign({
            lastError: ({ event }) => (event.error as Error)?.message || 'PAT authentication failed',
          }),
        },
      },
    },
        ...
        requesting_code: {
          invoke: {
            ...
            onError: {
              ...
              actions: assign({
                lastError: ({ event }) => (event.error as Error)?.message || 'Device code request failed',
              }),
            },
          },
        },
        waiting_for_user: {
          ...
          invoke: {
            ...
            onError: {
              target: '#auth.auth_failed',
              actions: assign({
                lastError: ({ event }) => (event.error as Error)?.message || 'Token polling failed',
              }),
            },
          },
          on: {
  ...
  actors: {
    ...
    requestDeviceCode: fromPromise(async ({ input }: { input: AuthContext }) => {
      ...
      return { deviceCode: 'mock-device-code', verificationUrl: 'https://microsoft.com/devicelogin' };
    }),
    pollForToken: fromPromise(async ({ input }: { input: AuthContext }) => {
      // Poll for auth completion
      return { token: 'mock-access-token', expiresAt: new Date(), refreshToken: 'mock-refresh-token' };
    }),
        ...
        waiting_for_panel: {
          ...
          on: {
            UPDATE_WEBVIEW_PANEL: {
              target: 'panel_ready',
              actions: ['updateWebviewPanelInContext', 'syncPendingDataIfAvailable', 'clearPendingData'],
            },
          },
        },
        panel_ready: {
  ...
  actions: {
    ...
    storeConnectionsFromSetup: assign(({ context, event }) => {
      ...
      for (const connection of setupResult.connections) {
        ...
        const resolvedAuthMethod: 'pat' | 'entra' = connection.authMethod === 'entra' ? 'entra' : previousState?.authMethod ?? 'pat';
    ...
    initializeConnectionActorsFromSetup: ({ context, event }) => {
      ...
      for (const [connectionId, actor] of Array.from(context.connectionActors.entries())) {
        const stillExists = connections.some((connection: ProjectConnection) => connection.id === connectionId);
        if (!stillExists) {
          try {
            actor.stop();
          } catch (error) {
            console.warn('[ApplicationFSM] Failed to stop connection actor during cleanup:', error);
          }
      ...
      for (const connection of connections) {
        ...
        if (existingActor) {
          try {
            ...
            console.warn('[ApplicationFSM] Failed to stop existing connection actor before recreation:', error);
          }
    ...
    updateWebviewPanelInContext: assign(({ context, event }) => {
      ...
      console.log('[ApplicationFSM] WEBVIEW_PANEL_TRACE - Non-UPDATE event, preserving context:', {
        eventType: event.type,
    ...
    updateConnectionState: assign(({ context, event }) => {
      console.log('[ApplicationFSM] WEBVIEW_PANEL_TRACE - updateConnectionState ENTRY:', {
        eventType: event.type,
        connectionId: event.type === 'CONNECTION_ESTABLISHED' ? event.connectionId : 'not applicable',
      ...
      const connectionStates = (() => {
        if (event.type === 'CONNECTION_ESTABLISHED' && event.connectionState) {
          ...
          console.log('[ApplicationFSM] updateConnectionState - AFTER storing connection state:', {
            connectionId: event.connectionId,
    ...
    syncPendingDataIfAvailable: ({ context }) => {
      console.log('[ApplicationFSM] syncPendingDataIfAvailable called - DETAILED context check:', {
      ...
      if (context.webviewPanel && context.pendingWorkItems) {
        console.log('[ApplicationFSM] Webview panel now available - syncing pending work items:', {
        ...
        if (context.webviewPanel?.webview?.postMessage) {
          try {
            ...
            console.error('[ApplicationFSM] Failed to sync pending work items to webview:', error);
          }
        }
      }
    },
    ...
    checkForDataLoading: ({ context }) => {
      const hasActiveConnection = !!(context.activeConnectionId && context.connectionStates.size > 0);
...
function activateConnection(context: ApplicationContext, connectionId: string) {
  ...
  if (context.isDeactivating) {
    console.log(`[ApplicationFSM] Skipping connection activation - extension is deactivating (connectionId: ${connectionId})`);
    return;
  }
...
function startAuthentication(context: ApplicationContext, connectionId: string) {
  ...
  if (context.isDeactivating) {
    console.log(`[ApplicationFSM] Skipping authentication - extension is deactivating (connectionId: ${connectionId})`);
    return;
  }
  ...
  switch ((message as { type?: unknown })?.type) {
    ...
    case 'timer:pause':
      if (context.timerActor) {
        ...
        if (timerEventType && timerEventType !== 'TICK' && timerEventType !== 'ACTIVITY' && timerEventType !== 'INACTIVITY_TIMEOUT' && timerEventType !== 'POMODORO_BREAK') {
...
async function performDeactivation({ input }: { input?: ApplicationContext }) {
  try {
    ...
    throw new Error(`Deactivation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function setupUI({ input }: { input: ApplicationContext }) {
  ...
  try {
    ...
    const activeResolution = resolveActiveConnectionId(persistedActiveId, normalization.connections);
