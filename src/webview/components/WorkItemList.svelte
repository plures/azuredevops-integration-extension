<script lang="ts">
  ...
  $: availableTypes = [...new Set(workItems.map((w: any) => w.fields?.['System.WorkItemType']).filter(Boolean))];
  $: availableStates = [...new Set(workItems.map((w: any) => normalizeState(w.fields?.['System.State'])).filter(Boolean))];
  
  // Filter and sort work items
  $: filteredItems = workItems.filter((item: any) => {
    ...
    switch (sortKey) {
      ...
      case 'updated-desc':
      default:
        return new Date(b.fields?.['System.ChangedDate'] || 0).getTime() - 
...
<div class="work-item-list">
  ...
  {:else}
    ...
    {#if filteredItems.length === 0 && workItems.length === 0}
      <div class="empty-state">
        <p>No work items loaded.</p>
        <button on:click={handleRefresh} class="primary-btn">Load Work Items</button>
      </div>
    ...
    {:else}
      <div class="items-container">
        {#each filteredItems as item (item.id)}
          <div class="work-item-card" on:click={() => handleOpenItem(item.id)} on:keydown role="button" tabindex="0">
            ...
            <div class="card-body">
              <div class="item-title">{item.fields?.['System.Title'] || `Work Item #${item.id}`}</div>
              
              <div class="item-meta">
                <span class="meta-badge type">{item.fields?.['System.WorkItemType'] || 'Task'}</span>
                <span class="meta-badge state state-{normalizeState(item.fields?.['System.State'])}">
