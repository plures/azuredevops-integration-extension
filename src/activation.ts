...
class AzureDevOpsIntViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(context: vscode.Extension.Context) {
    this.ctx = context;
  }
...
async function createPullRequestInteractive() {
  ...
  const repoPick = await vscode.window.showQuickPick(
    repos.map((r: any) => ({ label: r.name, description: r.id })),
...
async function showMyPullRequests() {
  ...
  if (!repo) {
    vscode.window.showWarningMessage('No repository available.');
    return;
  }
