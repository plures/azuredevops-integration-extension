...
var vscode, LogLevel, LOG_LEVEL_NAMES, FSMComponent, DEFAULT_CONFIG, FSMLogger, fsmLogger;
...
var IntelligentCache, WorkItemCache, workItemCache, apiCache, metadataCache;
var init_cache = __esm({
  "src/cache.ts"() {
    ...
    IntelligentCache = class {
       ...
       */
      getStats() {
        const topKeys = Array.from(this.cache.entries()).map(([key, entry]) => ({
          key,
    ...
    WorkItemCache = class _WorkItemCache extends IntelligentCache {
       ...
       */
      setWorkItems(key, workItems) {
        this.set(key, workItems, _WorkItemCache.WORK_ITEM_TTL);
      }
      /**
       * Cache query results with longer TTL
       */
      setQueryResult(key, result) {
        this.set(key, result, _WorkItemCache.QUERY_TTL);
      }
      /**
       * Cache metadata with longest TTL
       */
      setMetadata(key, metadata) {
        this.set(key, metadata, _WorkItemCache.METADATA_TTL);
      }
      /**
        ...
        this.axios.interceptors.response.use(
          ...
          async (err) => {
            ...
            if (status && (status === 429 || status >= 500 && status < 600) && attempt < maxAttempts) {
              ...
              const delay = Math.min(4e3, backoffBase + jitter);
              console.warn(
      ...
      async _getAuthenticatedIdentity() {
        ...
        try {
          ...
          const attemptConnectionData = async (basePath, desc, useAxiosBase) => {
            ...
            for (const version of _AzureDevOpsIntClient.connectionDataApiVersions) {
      ...
      async getWorkItems(query) {
        return await measureAsync("getWorkItems", async () => {
          ...
          try {
            ...
            const needsLimit = !knownUserScopedQueries.includes(query) && (["Recently Updated", "All Active", "Current Sprint"].includes(query) || !isKnownQuery);
            if (needsLimit) {
...
\u2022 Current URL: ${this.axios.defaults.baseURL}`;
      ...
      async getWorkItemTypeStates(workItemType) {
        try {
          ...
          return states.map((state) => state.name || state);
        } catch (e) {
      ...
      async getTeams() {
        return await measureAsync("getTeams", async () => {
          try {
            ...
            const orgTeamsUrl = this.baseUrl.replace(/\/$/, "") + `/_apis/projects/${this.encodedProject}/teams?api-version=7.0`;
...
async function validateClientConfig(context) {
  const { FSMComponent: FSMComponent2, fsmLogger: fsmLogger2, fsmContext } = await getConnectionLogger(context.connectionId);
  ...
  if (context.config.baseUrl && baseUrl !== context.config.baseUrl) {
    fsmLogger2.debug(FSMComponent2.CONNECTION, "Normalized baseUrl for client configuration", fsmContext, {
      original: context.config.baseUrl,
      normalized: baseUrl
    });
  }
  if (context.config.apiBaseUrl && apiBaseUrl !== context.config.apiBaseUrl) {
    fsmLogger2.debug(FSMComponent2.CONNECTION, "Normalized apiBaseUrl for client configuration", fsmContext, {
  ...
  if (errors.length > 0) {
    fsmLogger2.warn(FSMComponent2.CONNECTION, "Client configuration validation failed", fsmContext, { errors });
  ...
  fsmLogger2.debug(FSMComponent2.CONNECTION, "Client configuration validated successfully", fsmContext, {
...
async function createAzureClient(context, config) {
  ...
  try {
    const { AzureDevOpsIntClient: AzureDevOpsIntClient2 } = await Promise.resolve().then(() => (init_azureClient2(), azureClient_exports));
    const client = new AzureDevOpsIntClient2(
      config.organization,
      config.project,
      config.credential,
      config.options
    );
    fsmLogger2.info(FSMComponent2.CONNECTION, "Azure DevOps client created successfully", fsmContext, {
      organization: config.organization,
      project: config.project,
      authType: config.options.authType
    });
...
async function testClientConnectivity(context, client) {
  ...
  try {
    ...
    if (userId) {
      fsmLogger2.info(FSMComponent2.CONNECTION, "Client connectivity test successful", fsmContext, { userId });
      return { success: true, userId, context };
    } else {
      fsmLogger2.warn(FSMComponent2.CONNECTION, "Client connectivity test failed - no user ID", fsmContext);
      return { success: false, error: "Unable to authenticate user", context };
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    fsmLogger2.error(FSMComponent2.CONNECTION, "Client connectivity test failed", fsmContext, { error: errorMessage });
