...
async function diagnose() {
    ...
    console.log('\n🔍 Test 2: Simple Work Item Query...');
    try {
        const simpleQuery = 'SELECT [System.Id], [System.Title] FROM WorkItems ORDER BY [System.Id] DESC';
        ...
        if (simpleResults.length > 0) {
            console.log(`   Sample work item: #${simpleResults[0].id} - ${simpleResults[0].fields?.['System.Title'] || 'No title'}`);
        }
    } catch (error) {
    ...
    try {
        ...
        if (myActivityResults.length > 0) {
            console.log(`   Sample work item: #${myActivityResults[0].id} - ${myActivityResults[0].fields?.['System.Title'] || 'No title'}`);
        } else {
            console.log('   ℹ️  No work items in "My Activity" - this might be expected if you haven\'t created/modified work items recently');
        }
    } catch (error) {
    ...
    try {
        ...
        if (allActiveResults.length > 0) {
            console.log(`   Sample work item: #${allActiveResults[0].id} - ${allActiveResults[0].fields?.['System.Title'] || 'No title'}`);
        }
    } catch (error) {
    ...
    console.log('3. Ensure your PAT has sufficient permissions (Work Items Read, Project and Team Read)');
