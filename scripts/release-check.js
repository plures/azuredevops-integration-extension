...
function checkUnitTests() {
  ...
  if (failing === 0) {
    console.log(
      `  ✅ Unit tests passed: ${passing} passing${pending > 0 ? `, ${pending} pending` : ''} (20/20)`
    );
    return 20;
  }
  ...
  console.log(
    `  ⚠️  Unit tests: ${passing} passing, ${failing} failing${pending > 0 ? `, ${pending} pending` : ''} (${score}/20)`
  );
...
function checkCoverage() {
  ...
  try {
    ...
    if (fs.existsSync(coverageJsonPath)) {
      ...
      for (const file in coverageData) {
        ...
        // Functions coverage (f is an object with function ID as keys, execution count as values)
        if (fileCoverage.f) {
          const functions = fileCoverage.f;
          for (const funcId in functions) {
            totalFunctions++;
            if (functions[funcId] > 0) {
              coveredFunctions++;
            }
          }
        }
      ...
      if (totalLines > 0) {
        ...
        console.log(
          `  Lines: ${linesPct.toFixed(1)}% (${linesPct >= 85 ? '✅' : '❌'} 20pts), Branches: ${branchesPct.toFixed(1)}% (${branchesPct >= 80 ? '✅' : '❌'} 15pts), Functions: ${funcsPct.toFixed(1)}% (${funcsPct >= 80 ? '✅' : '❌'} 15pts)`
        );
  ...
  if (l === 0 && b === 0 && f === 0) {
    ...
    const summaryLines = out
      .split('\n')
      .filter(
        (line) => line.includes('All files') || line.includes('Summary') || line.includes('Total')
      );
