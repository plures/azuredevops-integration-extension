# Entra ID Authentication Migration Plan
...
- `src/services/auth/authentication.ts` - Authentication helper functions (getEntraIdToken, getPat)
...
- `src/praxis/connection/service.ts` - ConnectionService managing Praxis connection managers
- `src/praxis/connection/driver.ts` - ConnectionDriver handling connection lifecycle polling
...
- **Service layer pattern** (`AuthService`, `EntraAuthProvider`)
...
- [ ] Add unit tests for PKCE utilities (`tests/services/auth/pkceUtils.test.ts`)
...
export class EntraAuthProvider implements IAuthProvider {
  ...
  private async authenticateWithAuthCodeFlow(
...
describe('PKCE Utils', () => {
  it('should generate valid code verifier', () => {
    // Test code verifier format
  });

  it('should generate valid code challenge', () => {
    // Test code challenge derivation
  });
...
async authenticate(forceInteractive: boolean = false): Promise<AuthenticationResult> {
  try {
    if (this.useAuthCodeFlow) {
      return await this.authenticateWithAuthCodeFlow(forceInteractive);
    }
  } catch (error) {
  ...
  return await this.authenticateWithDeviceCodeFlow(forceInteractive);
}
```
