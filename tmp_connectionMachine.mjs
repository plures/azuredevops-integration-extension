...
async function discoverTenantFromOrganization(organizationUrl) {
  ...
  try {
    ...
    if (armResult) {
      logger.info('\u2705 Tenant discovered via ARM API', context, {
        tenantId: armResult,
        orgName,
      });
      return armResult;
              ...
              interactive_auth: {
                invoke: {
                  ...
                  onError: {
                    ...
                    actions: assign({
                      lastError: ({ event }) =>
                        event.error?.message || 'Entra authentication failed',
                      retryCount: ({ context }) => context.retryCount + 1,
                      reauthInProgress: false,
                      forceInteractive: () => false,
                    }),
...
export { AZURE_DEVOPS_PUBLIC_CLIENT_ID, AZURE_DEVOPS_SCOPE, connectionMachine };
