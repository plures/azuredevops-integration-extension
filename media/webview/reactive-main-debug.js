...
// src/webview/DebugApp.svelte
var on_click = () => {
  console.log("[DebugApp] Test button clicked");
  window.vscode?.postMessage({ type: "debug", message: "Webview is working!" });
};
var root = from_html(`<div style="padding: 20px; background: var(--vscode-editor-background); color: var(--vscode-editor-foreground); font-family: var(--vscode-font-family);"><h1>\u{1F389} Webview Debug Test</h1> <div style="margin: 10px 0;"><strong>Status:</strong> </div> <div style="margin: 10px 0;"><strong>Timestamp:</strong> </div> <div style="margin: 10px 0;"><strong>Svelte Version:</strong> 5.x (Runes)</div> <div style="margin: 10px 0;"><strong>Integration:</strong> FSM + Svelte 5 + VS Code</div> <button style="padding: 8px 16px; background: var(--vscode-button-background); color: var(--vscode-button-foreground); border: none; border-radius: 4px; cursor: pointer;">Test VS Code Communication</button> <div style="margin-top: 20px; padding: 10px; background: rgba(0,120,212,0.1); border-left: 3px solid var(--vscode-button-background);"><strong>\u2705 If you can see this, the webview is loading correctly!</strong> <br/> Next step: Check browser console for any JS errors.</div></div>`);
function DebugApp($$anchor, $$props) {
  push($$props, true);
  let mounted = state(false);
  let timestamp = state("");
  onMount(() => {
    set(mounted, true);
    set(timestamp, (/* @__PURE__ */ new Date()).toISOString(), true);
  ...
  reset(div_2);
  var button = sibling(div_2, 6);
  button.__click = [on_click];
  next(2);
  reset(div);
  template_effect(() => {
    set_text(text2, ` ${get(mounted) ? "\u2705 Mounted" : "\u23F3 Loading..."}`);
    set_text(text_1, ` ${get(timestamp) ?? ""}`);
  });
  append($$anchor, div);
  pop();
}
...
// src/webview/reactive-main-debug.ts
var vscode = window.vscode || window.acquireVsCodeApi?.();
...
try {
  ...
  console.log("[reactive-main] Debug app mounted successfully");
  window.__DEBUG_APP__ = {
    app,
    vscode
  };
} catch (error) {
...
//# sourceMappingURL=reactive-main-debug.js.map
